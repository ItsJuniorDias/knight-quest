 ...
 */
const torchLightsByRoom = new Map<string, { light: THREE.PointLight; seed: number }[]>();
const campfireLightsByRoom = new Map<string, { light: THREE.PointLight; seed: number }[]>();
...
function addTorch(
  ...
  if (!bucket) { bucket = []; torchLightsByRoom.set(roomKey, bucket); }
  bucket.push(rec);
}
...
function addCampfire(group: THREE.Group, roomKey: string, c: THREE.Vector3): void {
  ...
  if (!bucket) { bucket = []; campfireLightsByRoom.set(roomKey, bucket); }
  bucket.push(rec);
}
